#!/usr/bin/env python3
"""
STAGE Pipeline Deployment Automation Script
Automates deployment workflow for STAGE pipeline (master branch) with release tagging
"""
...
BUILD_DEFINITION_ID = "3308"  # STAGE pipeline
BRANCH = "master"  # STAGE pipeline uses master branch
...
def trigger_new_build(definition_id=None, branch=None, tag=None):
    ...
    if tag:
        ...
        # Get branch - use provided branch or default to STAGE branch
        if not branch:
            branch = BRANCH  # Default to "master" for STAGE
...
def get_pr_merges_after_commit(commit_hash, branch="dev"):
    ...
    try:
        ...
        # Get all commits from branch (without fromCommitId filter, as it's unreliable)
...
⏳ **Monitoring deployment progress...**
        ...
        if build_status.get('result') == 'partiallySucceeded':
            status_title = "✅ ** STAGE DEPLOYMENT COMPLETED**"
...
def main_deployment_workflow():
    ...
    # For STAGE pipeline: Use the commit from the latest tag (what was actually deployed/tagged)
    # Tags are always on 'master' branch
    # This ensures we only detect PRs that were actually deployed (tagged), not just built
    baseline_commit = build_info['source_version']  # Default: commit from last successful build
    
    # For STAGE pipeline, use the latest tag's commit (tags are on master branch)
    print(f"🏷️  Checking for latest tag to use as baseline...")
    latest_tag = get_latest_tag(REPOSITORY_NAME)
    if latest_tag:
        tag_commit = get_commit_from_tag(latest_tag, REPOSITORY_NAME)
        if tag_commit:
            print(f"✅ Using tag {latest_tag} commit as baseline: {tag_commit[:8]}")
            print(f"   (Instead of build {build_info['build_number']} commit: {baseline_commit[:8]})")
            baseline_commit = tag_commit
        else:
            print(f"⚠️  Could not get commit from tag {latest_tag}, using build's commit as baseline")
    else:
        print(f"⚠️  No tags found, using build's commit as baseline")
    ...
    # For STAGE pipeline, create a tag before triggering build
    tag_info = None
    if pr_merges:
        print(f"\n🏷️  Creating release tag for STAGE deployment...")
        tag_info = create_release_tag(pr_merges, branch=branch, repo_name=REPOSITORY_NAME)
        if not tag_info:
            print("⚠️  Warning: Failed to create tag, but continuing with build...")
    
    new_build_info = None
    if pr_merges:
        print(f"\n🔄 New changes detected! Triggering new build...")
        # For STAGE pipeline, use tag if available, otherwise use branch
        if tag_info and tag_info.get('tag_name'):
            print(f"   Using tag for STAGE pipeline: {tag_info['tag_name']}")
            new_build_info = trigger_new_build(def_id, branch=None, tag=tag_info['tag_name'])
        else:
            new_build_info = trigger_new_build(def_id, branch)
        if new_build_info:
            print(f"✅ New build {new_build_info['build_number']} triggered successfully!")
            if tag_info:
                print(f"   Tag: {tag_info.get('tag_name', 'N/A')}")
        else:
    ...
    if should_monitor:
        ...
        # which is from tag for STAGE pipeline)
        ...
        def run_monitoring():
            ...
            try:
                ...
                success = automated_deployment_workflow(prs_for_monitoring, current_build_info, approver_email=None, run_in_background=False, pipeline_name=pipeline_name, branch=branch, tag_info=tag_info)
                if success:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='STAGE Pipeline Deployment Automation Script',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Run full STAGE pipeline deployment workflow (default)
  python3 deployment_stage.py

  # Send approval request only
  python3 deployment_stage.py --approval --approver-email user@company.com

  # Send deployment confirmation
  python3 deployment_stage.py --deployment

  # Send approved message
  python3 deployment_stage.py --approved --approver-name "John Doe"

  # Send build triggered message
  python3 deployment_stage.py --build-triggered

  # Monitor specific build
  python3 deployment_stage.py --monitor --build-id 123456

  # Test PR detection (uses latest build commit)
  python3 deployment_stage.py --test-prs

  # Test PR detection with specific commit
  python3 deployment_stage.py --test-prs --commit a3cb1499
        """
    )
